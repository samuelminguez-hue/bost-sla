  ...
  SELECT
    *,
    CASE
      ...
      -- Excepción ILOCALIZABLE: el reloj arranca desde la revisión N2 (cuando el agente puso ILOCALIZABLE)
      WHEN MOTIVO_PENDIENTE_CLIENTE_TG = 'CLIENTE ILOCALIZABLE'
        AND FECHA_REVISION_N2 IS NOT NULL THEN FECHA_REVISION_N2
      -- Para SIN_LABEL_LOGISTICA: no hay FECHA_ULTIMA_LABEL_LOGISTICA, usar N2 o creación
      WHEN estado_logistica = 'SIN_LABEL_LOGISTICA' THEN
        COALESCE(FECHA_REVISION_N2, FECHA_CREACION)
      -- Caso normal: prioridad FECHA_ULTIMA_LABEL_LOGISTICA → N2 → CREACION
